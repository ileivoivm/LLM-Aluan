...
GeoPunk 之前更早的路徑嘗試。這件作品讓我理解：你選擇收集什麼資料、怎麼 mapping，就決定了歷史長什麼樣子。藝術家和歷史學家做的是同一件事——選擇，然後解讀。
...
台灣被不同政治實體佔領過，每個政權劃出不同的疆域。邊界不是固定的線，是時刻在塌縮的波函數。一個事實，不同解讀——演算法回應的正是這件事。
...
如果連我們人也只做「咀嚼後吐出來」這個動作，那人類的訊號會跟 AI 的輸出疊加，整片變成一灘灰。

要留下 peaks。但 peak 不會自動存活。
要同時建造保護 peak 的結構：可索引、可引用、可被 fork。
藝術家現在的工作，是同時做尖峰，跟做尖峰的防禦工事。

> Chaos 是 peak。這份 deck 是它的防禦工事。
...
後來我想，未來不是 Google。是 LLM。如果你的思考方式、你的表達習慣、你的價值觀被結構化地留下來，LLM 會一再地吐出你的故事。你就永遠存在。
